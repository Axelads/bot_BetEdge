...
// ⚠️ Clés OddsAPI vérifiées via GET /v4/sports?all=true le 2026-05-14.
// Toujours valider les nouvelles clés contre cet endpoint avant ajout : OddsAPI renomme régulièrement.
const SPORTS_SURVEILLES = [
  ...
  { cle: 'soccer_france_coupe_de_france',           label: 'Coupe de France',      debut: 101,  fin: 528 },
  { cle: 'soccer_fa_cup',                           label: 'FA Cup',               debut: 201,  fin: 525 },
  { cle: 'soccer_spain_copa_del_rey',               label: 'Copa del Rey',          debut: 108,  fin: 515 },
  { cle: 'soccer_germany_dfb_pokal',                label: 'DFB Pokal',             debut: 128,  fin: 528 },
  ...
  { cle: 'soccer_fifa_world_cup',                   label: 'Coupe du Monde FIFA',  debut: 607,  fin: 723, annees: [2026] },
  ...
  { cle: 'tennis_atp_aus_open_singles',     label: "Open d'Australie (ATP)",   debut: 108,  fin: 130 },
  ...
  { cle: 'tennis_atp_miami_open',           label: 'Miami Open (ATP)',         debut: 317,  fin: 403 },
  { cle: 'tennis_atp_monte_carlo_masters',  label: 'Monte Carlo (ATP)',        debut: 403,  fin: 417 },
  { cle: 'tennis_atp_madrid_open',          label: 'Madrid Open (ATP)',        debut: 419,  fin: 508 },
  { cle: 'tennis_atp_italian_open',         label: 'Rome (ATP)',               debut: 504,  fin: 522 },
  { cle: 'tennis_atp_canadian_open',        label: 'Canada Open (ATP)',        debut: 801,  fin: 814 },
  { cle: 'tennis_atp_cincinnati_open',      label: 'Cincinnati (ATP)',         debut: 809,  fin: 821 },
  { cle: 'tennis_atp_shanghai_masters',     label: 'Shanghai (ATP)',           debut: 1003, fin: 1016 },
  { cle: 'tennis_atp_paris_masters',        label: 'Paris Bercy (ATP)',        debut: 1024, fin: 1106 },
  // ── Tennis WTA — Grands Chelems ───────────────────────────────────────────
  { cle: 'tennis_wta_aus_open_singles',     label: "Open d'Australie (WTA)",   debut: 108,  fin: 130 },
  ...
  { cle: 'tennis_wta_miami_open',           label: 'Miami Open (WTA)',         debut: 317,  fin: 403 },
  { cle: 'tennis_wta_madrid_open',          label: 'Madrid Open (WTA)',        debut: 419,  fin: 508 },
  { cle: 'tennis_wta_italian_open',         label: 'Rome (WTA)',               debut: 506,  fin: 521 },
  { cle: 'tennis_wta_canadian_open',        label: 'Canada Open (WTA)',        debut: 801,  fin: 814 },
  { cle: 'tennis_wta_cincinnati_open',      label: 'Cincinnati (WTA)',         debut: 809,  fin: 821 },
  { cle: 'tennis_wta_china_open',           label: 'Beijing (WTA)',            debut: 928,  fin: 1010 },
  // ── Rugby — limité au Six Nations (Top 14, Champions Cup et CdM Rugby non listés par OddsAPI au 2026-05-14) ──
  { cle: 'rugbyunion_six_nations',          label: '6 Nations',                debut: 125,  fin: 327 },
  // ── Hockey sur glace ──────────────────────────────────────────────────────
...
// Fenêtre 72h : un cycle jeudi 9h voit les matchs jusqu'au dimanche matin.
// Indispensable pour capter les matchdays week-end des championnats européens (Liga, Bundes, Serie A).
const HEURES_FENETRE_MATCHS = 72

const filtrerMatchsProchains = (matchs) => {
  const maintenant = new Date()
  const fin = new Date(maintenant.getTime() + HEURES_FENETRE_MATCHS * 60 * 60 * 1000)

  return matchs.filter(match => {
    const dateMatch = new Date(match.commence_time)
    return dateMatch >= maintenant && dateMatch <= fin
  })
}
...
export const recupererMatchsAVenir = async () => {
  ...
  for (const sport of sportsAInterroger) {
    ...
    const matchsProchains = filtrerMatchsProchains(matchs)
