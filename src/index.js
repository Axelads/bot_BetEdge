...
// Pré-filtre global (large) — garde TOUS les matchs avec cotes valides pour enrichissement
// Pas de limite de nombre : toutes les compétitions actives sont interrogées (collecteurCotes.js)
// et tous les matchs valides sont enrichis (API-Football + buteurs + passeurs)
const filtrerMatchsGlobal = (matchs) => {
...
// Filtre par utilisateur — applique son profil de risque + ses sports
// Limite à 50 matchs max pour éviter une explosion du coût Claude
const filtrerMatchsUtilisateur = (matchs, prefBot) => {
  ...
  return matchs
    ...
    .slice(0, 50)  // max 50 matchs par user et par cycle
}
...
// ⚠️ Avertissement sécurité : MODE_PERMISSIF affaiblit les seuils de qualité
if (process.env.MODE_PERMISSIF === 'true') {
  console.warn('⚠️  [AVERTISSEMENT] MODE_PERMISSIF=true détecté — seuils d\'alerte affaiblis')
  console.warn('⚠️  Seuils réduits : score 60→45, prob 0.45→0.35, validation Claude bypassée')
  console.warn('⚠️  Garde-fou minimum conservé : edge >= 2% (fiabilité des cotes)')
  console.warn('⚠️  À utiliser UNIQUEMENT en démo/test, JAMAIS en production')
}
...
console.log('  • Marge bookmaker H2H : max 9% (déjà test de qualité)')
